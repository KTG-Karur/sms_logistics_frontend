...
const Index = () => {
    ...
    useEffect(() => {
        ...
        if (expenceTypeData && expenceTypeData.length > 0) {
            const transformed = expenceTypeData.map((item, index) => ({
                id: item.expenceTypeId,
                expence_type_name: item.expenceTypeName,
                is_active: item.isActive === 1 ? 'Active' : 'Inactive',
                originalData: item,
                isProtected: PROTECTED_EXPENSE_TYPES.includes(item.expenceTypeName)
            }));
    ...
    const getFilteredData = () => {
        if (!searchTerm.trim()) return transformedExpenceTypes;
        
        const searchLower = searchTerm.toLowerCase().trim();
        return transformedExpenceTypes.filter((expenseType) => {
            return (
                expenseType.expence_type_name?.toLowerCase().includes(searchLower)
            );
        });
    };
    ...
    const onEditForm = (data) => {
        ...
        setState({ expenceTypeName: data.expence_type_name });
        isEdit = true;
    ...
    const onFormSubmit = async (e) => {
        ...
        // Check for duplicate expence type name (case insensitive)
        ...
        try {
            ...
            if (isEdit) {
                dispatch(updateExpenceType({ request: requestData, expenceTypeId: selectedItem.id }));
            } else {
        ...
        <div>
            ...
            >
                <FormLayout 
                    ...
                    setState={setState} 
                    state={state} 
                    onChangeCallBack={{ handleInputChange: handleInputChange }} 
                    errors={errors} 
                    setErrors={setErrors} 
                    loading={loading} 
                />

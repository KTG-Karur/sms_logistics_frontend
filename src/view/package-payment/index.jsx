...
const PendingPayments = () => {
    ...
    // Check permissions
    const canPay = _.includes(accessIds, '9'); // Access ID 9 for payment
    const canView = _.includes(accessIds, '1'); // Access ID 1 for view
    ...
    const handleRecordPayment = (customer) => {
        if (!canPay) {
            showMessage('error', 'You do not have permission to record payments');
            return;
        }
    ...
    const handleViewShipments = async (customer) => {
        if (!canView) {
            showMessage('error', 'You do not have permission to view payment details');
            return;
        }
    ...
    const handlePrintBalancePayment = (customer) => {
        if (!canView) {
            showMessage('error', 'You do not have permission to print payment details');
            return;
        }
    ...
    const handleRowClick = (customer) => {
        if (parseFloat(customer.summary?.responsible_pending || 0) > 0 && canPay) {
            handleRecordPayment(customer);
        } else if (canView) {
            handleViewShipments(customer);
        } else {
            showMessage('info', 'You do not have permission to view or pay');
        }
    };
    ...
    const columns = [
        {
            ...
            Cell: ({ row }) => (
                <div 
                    className={`space-y-1 ${(canPay || canView) ? 'cursor-pointer hover:bg-gray-50' : ''} p-2 rounded-lg transition-colors duration-200`} 
        ...
        },
        {
            ...
            Cell: ({ row }) => (
                <div 
                    className={`space-y-2 ${(canPay || canView) ? 'cursor-pointer hover:bg-gray-50' : ''} p-2 rounded-lg transition-colors duration-200`} 
                    onClick={() => handleRowClick(row.original)}
                >
        ...
        },
        {
            ...
            accessor: 'actions',
            Cell: ({ row }) => {
                const hasPending = parseFloat(row.original.summary?.responsible_pending || 0) > 0;
            ...
            {!loading && filteredCustomers.length > 0 && !canView && !canPay && (
                <div className="text-center py-12 bg-white rounded-xl shadow-lg border border-gray-200">
                    <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gray-100 mb-4">
                        <IconEye className="w-10 h-10 text-gray-400" />
                    </div>
                    <h3 className="text-lg font-semibold text-gray-700 mb-2">Access Restricted</h3>
                    <p className="text-gray-500 mb-6 max-w-md mx-auto">You do not have permission to view or manage payment details.</p>

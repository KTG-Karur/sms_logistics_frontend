...
const Booking = () => {
  ...
  // Store current search input for each customer field
  const [senderSearchInput, setSenderSearchInput] = useState('');
  const [receiverSearchInput, setReceiverSearchInput] = useState('');
  ...
  // Combined customer dropdown with search by mobile number
  const getCombinedCustomerOptions = (searchValue, currentCustomerId) => {
    let filteredCustomers = customersData || [];
    
    // If search value is provided, filter by mobile number or name
    if (searchValue && searchValue.length > 0) {
      const searchLower = searchValue.toLowerCase();
      filteredCustomers = filteredCustomers.filter(cust => 
        (cust.customer_number && cust.customer_number.includes(searchValue)) ||
        (cust.customer_name && cust.customer_name.toLowerCase().includes(searchLower))
      );
    }
    
    const options = filteredCustomers.map((cust) => ({
      value: cust.customer_id,
      label: `${cust.customer_name} (${cust.customer_number})`,
      data: cust,
      mobileNo: cust.customer_number,
    }));
    
    // Add "Add New Customer" option if search value is a valid mobile number
    if (searchValue && searchValue.length === 10 && /^\d+$/.test(searchValue)) {
      // Check if customer with this mobile already exists
      const customerExists = customersData.some(cust => cust.customer_number === searchValue);
      if (!customerExists) {
        options.push({
          value: 'new',
          label: `+ Add New Customer (${searchValue})`,
          data: { id: 'new', name: 'New Customer', mobileNo: searchValue },
          mobileNo: searchValue,
          isNew: true,
        });
      }
    } else if (searchValue && searchValue.length > 0 && searchValue.length < 10 && /^\d+$/.test(searchValue)) {
      options.push({
        value: 'invalid',
        label: `Enter complete 10-digit mobile number`,
        data: { id: 'invalid' },
        isDisabled: true,
      });
    } else if (searchValue && searchValue.length > 0 && !/^\d+$/.test(searchValue)) {
      // If search is by name and no results found, show a message
      if (filteredCustomers.length === 0) {
        options.push({
          value: 'no-results',
          label: `No customers found. Enter mobile number to add new.`,
          data: { id: 'no-results' },
          isDisabled: true,
        });
      }
    ...
    // Add a placeholder option when no search and no customers
    if (options.length === 0 && !searchValue) {
      options.push({
        value: 'placeholder',
        label: 'Type mobile number or name to search...',
        data: { id: 'placeholder' },
        isDisabled: true,
      });
    }
  ...
  const handleFromCustomerSelect = (selected, actionMeta) => {
      ...
      Cell: ({ row }) => (
        <div className="space-y-1">
          ...
          <div className={`text-xs px-2 py-1 rounded-full inline-block ${
            row.original.payment_status === 'completed' ? 'bg-green-100 text-green-800' :
            row.original.payment_status === 'partial' ? 'bg-yellow-100 text-yellow-800' :
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 mb-4 sm:mb-6 animate-fadeIn">
          ...
          <form onSubmit={handleSubmit} className="p-3 sm:p-4">
            <div className="space-y-4 sm:space-y-5">
              ...
              {/* Row 2: Combined Customer Selection - Sender and Receiver */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Sender (Mobile/Customer) *</label>
                  <Select
                    options={getCombinedCustomerOptions(senderSearchInput, formData.fromCustomerId)}
                    onInputChange={(inputValue, actionMeta) => {
                      if (actionMeta.action !== 'input-blur' && actionMeta.action !== 'menu-close') {
                        setSenderSearchInput(inputValue);
                      }
                      return inputValue;
                    }}
                    value={getSelectedCustomerLabel(formData.fromCustomerId)}
                ...
                <div>
                  ...
                  <Select
                    options={getCombinedCustomerOptions(receiverSearchInput, formData.toCustomerId)}
                    onInputChange={(inputValue, actionMeta) => {
                      if (actionMeta.action !== 'input-blur' && actionMeta.action !== 'menu-close') {
                        setReceiverSearchInput(inputValue);
                      }
                      return inputValue;
                    }}
                    value={getSelectedCustomerLabel(formData.toCustomerId)}
                    ...
                    {formData.paymentBy === 'receiver' && (
                      <>
                        <div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-700">
                          Payment will be collected from receiver at delivery
                        </div>
                        <div>
                          <div className="flex items-baseline justify-between">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="flex flex-wrap gap-1">
                  <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                    selectedViewPackage.payment_status === 'completed' ? 'bg-green-100 text-green-800' :
                    selectedViewPackage.payment_status === 'partial' ? 'bg-yellow-100 text-yellow-800' :
                    'bg-gray-100 text-gray-800'
                  }`}>
                    {selectedViewPackage.payment_status || 'pending'}
                  </div>
                  <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                    selectedViewPackage.delivery_status === 'delivered' ? 'bg-green-100 text-green-800' :
